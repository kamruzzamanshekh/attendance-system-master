    ...
    $(document).ready(function() {
      ...
      $('#add_course').click(function() {
        ...
        if (c) {
          ...
          var old = $('#table_old_assigned').html();
          // alert(old);
          var html = "<tr><td hidden=\"\">" + teacher_id + "</td><td>" + teacher_name +
            "</td><td hidden=\"\">" + course_id + "</td><td>" + course_code + "-" + course_title +
            "</td><td><button type=\"button\" class=\"btn btn-outline-danger\" onclick=\"deleteOldAssigned(this)\">Delete</button></td></tr>";

          // alert(html);
          // alert(old.includes(html));
          // if (old.trim().indexOf(html.trim()) === -1) {
          if (!old.includes(html)) {
            // $('#tbody_new_course_assigned').html(old + html);
            html = "<tbody class='border-success new_assign'>" + html + "</tbody>";
            // alert(html);
            $('#table_old_assigned').html(old + html);

            var tbody = document.getElementsByClassName("new_assign");
            var tbody_count = tbody.length;

            for (var i = 0; i < tbody_count; i++) {
              var tr = tbody[i].getElementsByTagName("tr")[0];
              ...
              var result = "";
              var test_query_str = "course_info_idcourse_info=" + course_id_val + " AND teacher_info_idteacher_info=" + teacher_id_val + " AND ca_batch='" + ca_batch_val + "'";
              $.ajax({
                type: "POST",
                url: "insertAssignedCourse.php",
                data: {
                  test_query: test_query_str,
                  vals: val
                },
                success: function(msg) {
                  alert(msg);
      ...
      $('#assign_course').click(function() {
        ...
        alert("length " + len);
        alert("L2 " + tbody[0].getElementsByTagName("tr").length);
        for (var i = 0; i < len; i++) {
          // alert(tbody[i].getElementsByTagName("tr")[0].getElementsByTagName("td")[0].innerHTML);
          // alert("D");
        }
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="form-group col">
          <label>Session</label>
          <select class="custom-select" id="session" name="session" required autofocus>
      ...
      <div id="sec_assign_new_course" class="row">
        <div class="col">
          <div class="row">
            <div class="form-group col">
      ...
      <div id="div_old_assigned" class="row">
        <div class="col-lg-8 mx-auto">
          <h2 id="h2label">Course Assigned</h2>
